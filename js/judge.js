$(function() {
	...
	sercice('https://www.easy-mock.com/mock/5a5dc59cd467601e4b7f5731/game/judge',function(data){
		var teacherRecommend = data.body.teacherRecommend;
		console.log(teacherRecommend)
		var $ul = $('.ulList ul');
		$ul.empty();
		for (var i = 0; i < teacherRecommend.length; i ++) {			
			var thumb = teacherRecommend[i].thumb ? "http://artapp-dev-bucket.oss-cn-beijing.aliyuncs.com/" + teacherRecommend[i].thumb :"img/list.png";
			var userPhoto = teacherRecommend[i].userPhoto ? "http://artapp-dev-bucket.oss-cn-beijing.aliyuncs.com/" + teacherRecommend[i].userPhoto : "img/lufei.jpg";
